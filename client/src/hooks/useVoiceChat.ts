...
if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    console.log('🧹 [useVoiceChat] HMR cleanup');
...
if (typeof window !== 'undefined') {
  const cleanup = () => {
    console.log('🧹 [useVoiceChat] Page unload cleanup');
    if (globalWebRTCManager) {
      (globalWebRTCManager as any).close();
      globalWebRTCManager = null;
      currentManagerRoomCode = null;
...
export const useVoiceChat = (
  ...
  // WebRTC 연결 인원수 모니터링
  useEffect(() => {
    const connectedPeers = remoteStreams.size;
    const totalInRoom = connectedPeers + (localStream ? 1 : 0); // 로컬 스트림까지 포함
    
    console.log(`🌐 [WebRTC] 연결 상태 업데이트:`, {
      remoteStreams: connectedPeers,
      hasLocalStream: !!localStream,
      totalWebRTCParticipants: totalInRoom,
      streamDetails: Array.from(remoteStreams.entries()).map(([socketId, stream]) => ({
        socketId: socketId,
        streamId: stream.id,
    ...
    console.log(`📡 [WebRTC] 현재 WebRTC 참여자: ${totalInRoom}명 (본인 ${localStream ? '포함' : '제외'}, 원격 ${connectedPeers}명)`);
  ...
  useEffect(() => {
    ...
    managerInitPromise.then(manager => {
      ...
      manager.onUserLeft = (socketId) => {
        if (isMounted) {
          console.log(`👋 [useVoiceChat] User left processing:`, { 
            socketId, 
            currentStreams: remoteStreams.size 
          });
          
          setRemoteStreams(prev => {
            const newStreams = new Map(prev);
            const removed = newStreams.delete(socketId);
            console.log(`🗺️ [useVoiceChat] Stream removed:`, {
              socketId,
              wasRemoved: removed,
              remainingStreams: newStreams.size,
              remainingKeys: Array.from(newStreams.keys())
            });
            return newStreams;
          });
          
          // 방 상태 새로고침 (참여자 목록 업데이트)
          if (onStateUpdate) {
            console.log('🔄 [useVoiceChat] Triggering room state update after user left');
            setTimeout(onStateUpdate, 500);
          }
        }
      };
      ...
      manager.onParticipantUpdate = (event: ParticipantUpdateEvent) => {
        if (isMounted) {
          ...
          // ✨ 실시간 방 상태 업데이트 호출
          if (onStateUpdate) {
            console.log("🔄 [useVoiceChat] 참가자 업데이트로 인한 방 상태 새로고침 요청");
            onStateUpdate();
          }
        }
      };
      ...
      manager.onRoomUsers = (users: unknown[]) => {
        if (isMounted) {
          ...
          // ✨ 실시간 방 상태 업데이트 호출
          if (onStateUpdate) {
            console.log("🔄 [useVoiceChat] 방 사용자 업데이트로 인한 방 상태 새로고침 요청");
            onStateUpdate();
          }
        }
      };
