...
export const useRealTimeRoom = ({ roomCode, currentPlayer, enabled }: RealTimeRoomHookProps) => {
  ...
  useEffect(() => {
    ...
    console.log('🚀 실시간 방 기능 초기화 시작:', { roomCode, nickname: currentPlayer.nickname });
    ...
    socket.on('connect', () => {
      ...
      if (userId) {
        console.log('📡 방 실시간 이벤트 구독:', { roomCode, userId, nickname: currentPlayer.nickname });
