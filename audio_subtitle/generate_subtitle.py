...
def detect_voice_activity(audio_path, frame_length=2048, hop_length=512, top_db=30):
    """
    오디오에서 음성 활동 구간을 감지하여 정확한 시작/끝 시점 반환 (완전 무료!)
    """
    try:
        # 오디오 로드
...
def transcribe_with_improved_timing(audio_path, model="whisper-1"):
    """
    OpenAI Whisper API + 무료 VAD를 사용하여 정확한 타이밍으로 자막 생성
    """
    try:
            ...
            for whisper_seg in response["segments"]:
                ...
                if best_match and best_overlap > 0.1:  # 최소 0.1초 겹침
                    ...
                    adjusted_segments.append({
                        ...
                        "time_saved": whisper_end - adjusted_end,
                        "adjusted": True
                    })
                else:
                    # VAD 매칭이 안되면 원본 사용
...
def transcribe_with_api(audio_path, model="whisper-1"):
    """
    OpenAI Whisper API를 사용하여 음성을 텍스트로 변환 (기본 버전)
    """
    try:
        with open(audio_path, "rb") as audio_file:
            response = openai.Audio.transcribe(
...
def save_srt(segments, out_path):
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        for i, segment in enumerate(segments, 1):
            ...
            # 조정 여부와 절약된 시간 표시
            if segment.get("adjusted") and segment.get("time_saved", 0) > 0:
                time_saved = segment.get("time_saved", 0)
                text += f" [⚡{time_saved:.1f}초 단축]"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  --improved-timing: 무료 VAD를 사용하여 자막 끝 시점 정확도 개선")
        sys.exit(1)
    ...
    if use_improved_timing:
        print("🔍 OpenAI Whisper API + 무료 VAD 타이밍 개선으로 변환 중...")
        print("   💡 VAD(Voice Activity Detection)는 완전 무료 라이브러리입니다!")
        result = transcribe_with_improved_timing(audio_file)
        ...
        if result:
            ...
            total_time_saved = sum(seg.get("time_saved", 0) for seg in result["segments"])
            
            print(f"\n📊 타이밍 개선 결과:")
            print(f"   🔧 조정된 세그먼트: {adjusted_count}/{total_count}개")
            print(f"   ⚡ 총 단축된 시간: {total_time_saved:.1f}초")
            print(f"   🎤 감지된 음성 구간: {len(result.get('voice_activity_segments', []))}개")
            
            if total_time_saved > 0:
                print(f"\n🎯 원래 Whisper가 {total_time_saved:.1f}초 더 길게 잡았던 것을 정확하게 조정했습니다!")
        ...
        if text:
            ...
            print("\n💡 팁: --improved-timing 옵션으로 자막 끝 시점을 정확하게 조정할 수 있습니다!")
            print("   (VAD는 완전 무료 라이브러리로 추가 비용 없음)")
        else:
            print("❌ 음성 변환에 실패했습니다.") 